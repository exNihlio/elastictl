...
def main():
    ...
    parser.add_argument("--http-port", help="The HTTP port of the \
    ...
    if args.addr:
        ...
        argdict['addr'] = "localhost"

    if args.http-port:
        argdict['http-port'] = args.http-port
    else:
        argdict['http-port'] = 9200
...
def endPointConstructor(argdict):
    ...
    if argdict['http-port'] == 9200:
        pass
    elif argdict['http-port'] <= 1024: 
        # Log warning the Elasticsearch port we're connecting to 
        # conflicts with other ports.
        print("Warning: 'http-port' uses a well-known port")
        print("Your Elasticsearch node may conflict with other services")
        baseEndpoint = baseEndpoint + str(argdict['http-port'])
    else:
        baseEndpoint = baseEndpoint + str(argdict['http-port'])
